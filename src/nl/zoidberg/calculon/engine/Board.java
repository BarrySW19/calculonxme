...
public class Board {
    ...
    public boolean isInCheck() {
    	return (inCheck == null ? (inCheck = new Boolean(CheckDetector.alreadyInCheck(this))) : inCheck).booleanValue();
    }
    ...
    public String getResult() {
        ...
        if( ! isInCheck()) {
            ...
            long possiblePins = Bitmaps.starMap[(kingPos[0]<<3)|kingPos[1]] & ~BitBoard.getFileMap(kingPos[0]) & ~Bitmaps.BORDER & getBitBoard().getBitmapColor(getPlayer());
    ...
    public Board applyMove(String algebraic) {
        ...
        if((movePiece & Piece.MASK_TYPE) == Piece.PAWN && isEnPassant()
        	...
        	getBitBoard().clearSquare(toFile, fromRank, squares[toFile<<3|fromRank]);
        ...
        getBitBoard().clearSquare(fromFile, fromRank, squares[fromFile<<3|fromRank]);
        getBitBoard().clearSquare(toFile, toRank, squares[toFile<<3|toRank]);
        ...
        if((movePiece & Piece.MASK_TYPE) != Piece.PAWN || (toRank != 0 && toRank != 7)) {
            ...
            getBitBoard().populateSquare(toFile, toRank, movePiece);
        } else {
            ...
            getBitBoard().populateSquare(toFile, toRank, squares[toIdx]);
        }
